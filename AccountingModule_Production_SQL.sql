...
-- Version: 1.2 (Added USE database and verification)
-- Date: 2025-12-30
...
-- NOTE: This script runs as a single transaction.
-- If any part fails, the entire script will be rolled back.
...
BEGIN TRY
    ...
    -- =====================================================
    -- 9. Create __EFMigrationsHistory table if it doesn't exist
    -- =====================================================
    ...
    ELSE
    BEGIN
        PRINT 'Table __EFMigrationsHistory already exists';
    END

    -- =====================================================
    -- 10. Insert migration record (for EF Core tracking)
    -- =====================================================
    ...
    -- =====================================================
    PRINT '';
    PRINT 'VERIFICATION - Tables in database ' + DB_NAME() + ':';

    IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[LedgerAccounts]') AND type in (N'U'))
        PRINT '  [OK] LedgerAccounts exists';
    ELSE
        PRINT '  [MISSING] LedgerAccounts NOT FOUND!';

    IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[JournalEntries]') AND type in (N'U'))
        PRINT '  [OK] JournalEntries exists';
    ELSE
        PRINT '  [MISSING] JournalEntries NOT FOUND!';

    IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[JournalEntryLines]') AND type in (N'U'))
        PRINT '  [OK] JournalEntryLines exists';
    ELSE
        PRINT '  [MISSING] JournalEntryLines NOT FOUND!';

    IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AccountingPeriods]') AND type in (N'U'))
        PRINT '  [OK] AccountingPeriods exists';
    ELSE
        PRINT '  [MISSING] AccountingPeriods NOT FOUND!';
...
BEGIN CATCH
    ...
    PRINT '';
    PRINT '=====================================================';
    PRINT 'ERROR: Accounting Module migration FAILED!';
    PRINT '=====================================================';
