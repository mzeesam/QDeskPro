...
- **PWA**: Progressive Web App with installability and push notifications
...
QDeskPro is built as a PWA to provide a native app-like experience for clerks using tablets and mobile devices in the field.

### PWA Capabilities

| Feature | Description |
|---------|-------------|
| **Installable** | Users can install the app to their home screen on mobile/desktop |
...
| **Asset Caching** | Fast load times with cached static assets |

### Web App Manifest (manifest.json)

```json
{
  "name": "QDeskPro - Quarry Management",
  "short_name": "QDeskPro",
  "description": "Quarry sales and operations management system",
  "start_url": "/",
  "display": "standalone",
  ...
  "shortcuts": [
    ...
    },
    {
      ...
      "url": "/clerk/reports",
      "icons": [{ "src": "/icons/report-shortcut.png", "sizes": "96x96" }]
    }
  ]
}
```
...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="QDeskPro" />
    <meta name="mobile-web-app-capable" content="yes" />
    <meta name="application-name" content="QDeskPro" />
...
In `App.razor` or a JavaScript file:
...
{
    Task<bool> SubscribeAsync(string userId, PushSubscription subscription);
    Task<bool> UnsubscribeAsync(string userId);
    Task SendNotificationAsync(string userId, NotificationPayload payload);
    Task SendToQuarryAsync(string quarryId, NotificationPayload payload);
}
...
public class PushNotificationService : IPushNotificationService
{
    ...
    public PushNotificationService(IConfiguration config, AppDbContext context)
    {
        _config = config;
        _context = context;
    }

    public async Task SendNotificationAsync(string userId, NotificationPayload payload)
    {
...
public record NotificationAction(string Action, string Title);
```

### Notification Use Cases

```csharp
// Example: Notify manager when daily report is ready
await _pushService.SendToQuarryAsync(quarryId, new NotificationPayload(
    Title: "Daily Report Ready",
    Body: $"Sales report for {DateTime.Today:dd/MM/yyyy} is ready for review",
    Url: "/reports/daily-sales"
));

// Example: Notify clerk of unpaid orders reminder
await _pushService.SendNotificationAsync(clerkId, new NotificationPayload(
    Title: "Unpaid Orders Reminder",
    Body: $"You have {unpaidCount} unpaid orders totaling KES {unpaidAmount:N0}",
    Url: "/clerk/reports",
    Actions: new[]
    {
        new NotificationAction("view", "View Orders"),
...
{
    <MudAlert Severity="Severity.Info" Class="install-prompt">
        <MudStack Row="true" AlignItems="AlignItems.Center" Justify="Justify.SpaceBetween">
            <MudText>Install QDeskPro for faster access and offline support</MudText>
            <MudStack Row="true" Spacing="2">
                <MudButton Variant="Variant.Text" OnClick="DismissPrompt">Later</MudButton>
                <MudButton Variant="Variant.Filled" Color="Color.Primary" OnClick="InstallApp">
                    Install
                </MudButton>
            </MudStack>
        </MudStack>
...
@code {
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _showInstallPrompt = await JS.InvokeAsync<bool>("canInstallPwa");
            StateHasChanged();
        }
    }
...
### PWA Configuration in Program.cs

```csharp
// Program.cs - PWA configuration
...
// Enable static file caching for PWA
...
{
  "Vapid": {
    "Subject": "mailto:admin@qdeskpro.com",
...
### PWA Entity for Push Subscriptions
...
- [ ] App is installable on Chrome/Edge
...
- [ ] App loads when offline (cached pages only)
